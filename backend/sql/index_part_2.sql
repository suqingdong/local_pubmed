...
CREATE INDEX idx_ts_en_part_2 ON pubmed_part_2 USING gin (ts_en);

-- 4. 常用字段 B-tree 索引 (构建极快，通常几分钟)
-- pmid 索引
CREATE INDEX idx_pmid_part_2 ON pubmed_part_2 (pmid);
-- year 索引
CREATE INDEX idx_year_part_2 ON pubmed_part_2 (year);
-- factor 索引 (影响因子)
CREATE INDEX idx_factor_part_2 ON pubmed_part_2 (factor);

-- 5. 验证当前分区的索引是否全部创建成功
SELECT indexrelname, pg_size_pretty(pg_relation_size(indexrelid)) 
FROM pg_stat_user_indexes 
WHERE relname = 'pubmed_part_2';