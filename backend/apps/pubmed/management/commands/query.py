...
def hybrid_search(query,
    ...
    loguru.logger.debug(f"📝 BM25 召回耗时: {time.time() - time_b:.2f}s，命中 {len(bm25_list)} 条。")
    ...
    loguru.logger.debug(f"🔢 向量召回耗时: {time.time() - time_v:.2f}s，命中 {len(vector_list)} 条。")
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument('--year', help='filter by year', type=int, default=2021)
